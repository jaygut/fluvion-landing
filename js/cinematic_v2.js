...
(function () {
  ...
  const CINE = (window.__CINE = {
    ready: false, scene: "0", nScenes: 15, errors: [],
  ...
  // ---- scenes: keyed by the section's data-scene STRING (not a positional index), so inserting a
  //      scene never renumbers. camera + per-scene mood. emphasis: arcs/proof/src/keystone drive the
  //      deck layers; shed drives the isolines; emit/turb/bloom are read by atmosphere.js, which also
  //      keys its grade HUD off __CINE.scene === "6" (grade) / "7" (monitor). Every entry MUST carry
  //      every EKEY (emOf/lerp read them all); a missing key would lerp to NaN.
  const SCENES = {
    "0":               { v: { longitude: -57, latitude: -16, zoom: 3.3, pitch: 35, bearing: -8 }, arcs: .9, proof: 0, src: 1, shed: .5, keystone: 0, context: 0, emit: .65, turb: .35, bloom: .5 },
    "1":               { v: { longitude: -57, latitude: -16, zoom: 3.05, pitch: 28, bearing: 0 }, arcs: .7, proof: 0, src: 1, shed: .4, keystone: 0, context: 0, emit: .4, turb: .3, bloom: .4 },
    "2":               { v: { longitude: -66.5, latitude: -6.2, zoom: 5.1, pitch: 46, bearing: 18 }, arcs: .4, proof: 0, src: 1.7, shed: .85, keystone: 0, context: 0, emit: 1, turb: .25, bloom: .7 },
    "3":               { v: { longitude: -60, latitude: -18, zoom: 3.6, pitch: 52, bearing: -14 }, arcs: .9, proof: 0, src: 1.2, shed: 1, keystone: 0, context: 0, emit: 1, turb: .5, bloom: .85 },
    "g-graph":         { v: { longitude: -62, latitude: -13.5, zoom: 3.9, pitch: 46, bearing: -10 }, arcs: .5, proof: 0, src: .8, shed: .15, keystone: .38, context: 0, emit: .45, turb: .3, bloom: .45 },
    "g-keystone":      { v: { longitude: -63, latitude: -10, zoom: 4.5, pitch: 44, bearing: 6 }, arcs: .08, proof: 0, src: .3, shed: 0, keystone: 1, context: 0, emit: .32, turb: .22, bloom: .5 },
    "g-keystone-audit": { v: { longitude: -63, latitude: -10, zoom: 4.5, pitch: 44, bearing: 6 }, arcs: .08, proof: 0, src: .3, shed: 0, keystone: 1, context: 0, emit: .3, turb: .22, bloom: .5 },
    "s-seasonal":      { v: { longitude: -63, latitude: -12, zoom: 4.0, pitch: 38, bearing: 10 }, arcs: .5, proof: 0, src: 1.5, shed: .8, keystone: .2, context: 0, emit: .65, turb: .3, bloom: .6 },
    "4":               { v: { longitude: -53, latitude: -27, zoom: 4.5, pitch: 40, bearing: 2 }, arcs: .35, proof: 1, src: .6, shed: .45, keystone: 0, context: 0, emit: .6, turb: .45, bloom: .5 },
    "s-context":       { v: { longitude: -54.2, latitude: -23.5, zoom: 4.65, pitch: 42, bearing: -8 }, arcs: .18, proof: .28, src: .28, shed: .22, keystone: 0, context: 1, emit: .34, turb: .25, bloom: .38 },
    "5":               { v: { longitude: -67.25, latitude: -6, zoom: 6, pitch: 50, bearing: 24 }, arcs: .3, proof: 0, src: 2, shed: .8, keystone: 0, context: 0, emit: .7, turb: .3, bloom: .6 },
    "6":               { v: { longitude: -65.8, latitude: -6.6, zoom: 5.5, pitch: 52, bearing: 16 }, arcs: .25, proof: 0, src: 2.2, shed: .55, keystone: 0, context: 0, emit: 1, turb: .45, bloom: .7 },
    "7":               { v: { longitude: -64, latitude: -9, zoom: 4.3, pitch: 46, bearing: 14 }, arcs: .3, proof: 0, src: 2.2, shed: .6, keystone: 0, context: 0, emit: .85, turb: .55, bloom: .6 },
    "s-global":        { v: { longitude: -58, latitude: -14, zoom: 2.9, pitch: 18, bearing: 0 }, arcs: .6, proof: .25, src: .8, shed: .5, keystone: 0, context: 0, emit: .5, turb: .3, bloom: .5 },
    "8":               { v: { longitude: -57, latitude: -17, zoom: 3.2, pitch: 24, bearing: 0 }, arcs: .9, proof: 0, src: 1, shed: .4, keystone: 0, context: 0, emit: .9, turb: .35, bloom: .5 },
  };
  const EKEYS = ["arcs", "proof", "src", "shed", "keystone", "context", "emit", "turb", "bloom"];
  // keystone field cells from the scrubbed public export (normalized weight, no magnitudes, no parcels)
  const KS = window.FLUVION_KEYSTONE;
  const ksCells = (KS && Array.isArray(KS.cells)) ? KS.cells : [];
  const CTX = window.FLUVION_SOY_CONTEXT;
  const ctxCells = (CTX && Array.isArray(CTX.cells)) ? CTX.cells : [];
  const CTX_FIELD = { soy: 2, drought: 3, heatsoil: 4, overlap: 6 };
  function ctxMode() {
    const m = window.__SOY_CONTEXT && window.__SOY_CONTEXT.mode;
    return CTX_FIELD[m] != null ? m : "overlap";
  }
  function ctxColor(mode, t, alpha) {
    const a = Math.max(0, Math.min(245, alpha));
    if (mode === "soy") return [lerp(28, 96, t), lerp(148, 238, t), lerp(126, 190, t), a];
    if (mode === "drought") return [lerp(122, 246, t), lerp(75, 142, t), lerp(56, 82, t), a];
    if (mode === "heatsoil") return [lerp(105, 246, t), lerp(96, 207, t), lerp(128, 95, t), a];
    return [lerp(80, 255, t), lerp(140, 231, t), lerp(156, 180, t), a];
  }
  ...
  function buildLayers(em, time) {
    ...
    // keystone field: the null-audited first-order dependency per cell, coral (faint) -> warm-white
    // (hot). The bright minority IS "position sets weight". Normalized 0..1; no magnitudes ship.
    if (em.keystone > 0.02 && ksCells.length) {
      layers.push(new ScatterplotLayer({
        id: "keystone", data: ksCells, getPosition: (d) => [d[0], d[1]], radiusUnits: "pixels",
        getRadius: 7, getFillColor: (d) => { const t = d[2];
          return [lerp(226, 255, t), lerp(96, 238, t), lerp(70, 196, t), (26 + 210 * Math.pow(t, 0.7)) * em.keystone]; },
        updateTriggers: { getFillColor: [em.keystone] },
      }));
    }
    // Soy context: public visual scores only (no per-cell dollars, no repricing). The mode is
    // controlled by soy_context.js; default overlap keeps the map useful before the widget hydrates.
    if (em.context > 0.02 && ctxCells.length) {
      const mode = ctxMode(), field = CTX_FIELD[mode] || CTX_FIELD.overlap;
      layers.push(new ScatterplotLayer({
        id: "soy-context", data: ctxCells, getPosition: (d) => [d[0], d[1]], radiusUnits: "pixels",
        getRadius: (d) => 2.2 + 9.5 * Math.sqrt(Math.max(0, d[field] || 0)),
        getFillColor: (d) => {
          const t = Math.max(0, Math.min(1, d[field] || 0));
          return ctxColor(mode, Math.pow(t, 0.72), (38 + 205 * Math.pow(t, 0.82)) * em.context);
        },
        stroked: true,
        getLineColor: (d) => {
          const t = Math.max(0, Math.min(1, d[field] || 0));
          return [255, 255, 255, (14 + 75 * t) * em.context];
        },
        lineWidthUnits: "pixels", getLineWidth: 0.35,
        updateTriggers: { getRadius: [mode], getFillColor: [mode, em.context], getLineColor: [mode, em.context] },
      }));
    }
  ...
  function drawFallback(reason) {
    ...
    if (CINE.scene === "s-context" && ctxCells.length) {
      const mode = ctxMode(), field = CTX_FIELD[mode] || CTX_FIELD.overlap;
      const ctx = svg.append("g").attr("opacity", 0.94);
      ctx.selectAll("circle").data(ctxCells).join("circle")
        .attr("cx", (d) => P(d[0], d[1])[0]).attr("cy", (d) => P(d[0], d[1])[1])
        .attr("r", (d) => 1.6 + 5.2 * Math.sqrt(Math.max(0, d[field] || 0)))
        .attr("fill", (d) => { const t = Math.pow(Math.max(0, Math.min(1, d[field] || 0)), 0.72);
          const c = ctxColor(mode, t, 205); return `rgba(${c[0]},${c[1]},${c[2]},${c[3] / 255})`; })
        .attr("stroke", "rgba(255,255,255,.18)").attr("stroke-width", 0.25);
    }
  ...
  function go(key) {
    const sc = SCENES[key] || SCENES["0"];   // unknown data-scene falls back safely, no crash
    CINE.scene = key;
    if (!CINE.useDeck) {
      CINE.emphasis = emOf(sc); CINE.viewState = sc.v;
      fbDrawn = false; d3.select(wrap).selectAll("svg.fallback").remove(); drawFallback(CINE.fallback || "no-webgl");
      return;
    }
    from = { ...cur }; emFrom = { ...emCur }; target = sc; t0 = performance.now();
  }
  ...
  function setLegend(key) {
    if (key === "4") { legend.innerHTML = '<i style="background:#e63946"></i>soy lost &nbsp; <i style="background:#f4d35e"></i>steady &nbsp; <i style="background:#43aa8b"></i>resilient'; legend.classList.add("show"); }
    else legend.classList.remove("show");
    if (illus) {
      const hideMobileContext = key === "s-context" && window.innerWidth < 700;
      if (ILLUS[key] && !hideMobileContext) { illus.innerHTML = "<b>Illustrative.</b> " + ILLUS[key]; illus.classList.add("show"); }
      else illus.classList.remove("show");
    }
  }
  ...
  scroller.setup({ step: ".step", offset: 0.6 }).onStepEnter(({ element, index }) => {
    ...
    const key = element.dataset.scene; go(key); setLegend(key);
