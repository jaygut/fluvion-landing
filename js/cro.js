...
(function () {
  ...
  var BADGE = {
    scenario_simulation: "an if-then valuation: if the forest rain-making falls by a set amount, what is the loss and the value",
    concurrent_hindcast: "does the engine recover which regions fell in the observed drought years",
    descriptive_monitoring: "the observed current condition of the source forest, beside the price"
  };
  var badges = C.claim_badges.map(function (b) {
    return '<span class="cro-badge" title="' + (BADGE[b] || "") + '">' + b.replace(/_/g, " ") + "</span>";
  }).join("");
  ...
  el.innerHTML =
    ...
    '<div class="cro-scn"><span class="cro-scn-lab">Stress the source</span>' +
      ...
      '<span class="cro-scn-note">f_loss (the assumed loss of rain-making) is a named stress, not a probability</span></div>' +
    '<div class="cro-grid">' +
      ...
      '<div class="cro-cell cro-cell--muted" title="Value-at-Risk (VaR95): a high-end estimate of the yearly loss, at the 95th percentile"><div class="cro-k">UK loss at risk &middot; VaR95</div><div class="cro-val">' + usdM(v.var95_usd) + '<span class="cro-flag">placeholder</span></div>' +
